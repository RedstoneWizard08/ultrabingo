...
public static class BingoCard {
    ...
    public static void UpdateTitles(int gameType) {
        var bingoDescription = "Race to <color=orange>obtain the fastest time</color> for your team on each level."
                               + "\nClaim " + GameManager.CurrentGame.grid.size +
                               " levels horizontally, vertically or diagonally for your team to win!";
        ...
        if (_teamIndicator != null)
            _teamIndicator.GetComponent<TMP_Text>().text =
                "-- You are on the <color=" + GameManager.CurrentTeam?.ToLower() +
                ">" + GameManager.CurrentTeam + " team</color> --";
