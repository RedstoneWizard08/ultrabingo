...
public static class OptionsMenuPatcher {
    ...
    public static void DisableMajorAssistsOptions(ref SettingsPage settingsPage) {
        if (GameManager.IsInBingoLevel && !GameManager.CurrentGame.isGameFinished()) {
            ...
            try {
                GetGameObjectChild(GetGameObjectChild(optionsMenu.gameObject, "Navigation Rail"),
                    "PluginConfiguratorButton(Clone)").SetActive(false);
            } catch (Exception e) {
            ...
            if (settingsPage.name == "Assist") {
                ...
                var disabledNotification =
                    UIHelper.CreateText(
                        "Major assists are <color=orange>disabled</color> while playing Baphomet's Bingo.", 26,
                        "TextDisabled");
